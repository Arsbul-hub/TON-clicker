    ...
    def buy(self, name):
        ...
        if name in self.store_items:
            ...
            if index < self.store_items[name]["index"]:
                ...
                Snackbar(text="Этот предмкт хуже, чем у вас есть!", duration=.2).open()
            elif index == self.store_items[name]["index"]:
                Snackbar(text="У вас уже есть этот предмет!", duration=.2).open()
        ...
        #         Snackbar(text="У вас уже усть эта мышь!",duration=.2).open()
            ...
            if self.player_data["TON"] - self.player_data["chest"]["price"] >= 0:
                ...
                self.player_data["chest"]["price"] += self.player_data["chest"]["price"] / 100 * 20
                self.open_chest()
            else:
                # self.dialog.dismiss()
                Snackbar(text="У вас не хватает на это TON!", duration=.2).open()
...
''')
    ...
    def open_chest(self):
        ...
        bonuse_items = self.bonuses[int(type_index / 10)]
